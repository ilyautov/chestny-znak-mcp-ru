#!/usr/bin/env python3
"""Запускалка chestny-znak-mcp-ru, поднимающая себе окружение сама.
...
текущему процессу и стартует сервер. Дальше запуск мгновенный.
    ...
    python3 serve.py crpt               # запустить сервер
    python3 serve.py crpt --selfcheck   # проверить установку и выйти

Нужна именно она, а не `uvx chestny-znak-mcp-ru`: бандл .mcpb для Claude Desktop
...
VENV = Path(os.environ.get("CRPT_MCP_VENV", HERE / ".venv"))
DEPS = ["schema-mcp-core>=0.2.0,<1"]
SERVICES = {"crpt": "crpt_mcp.server"}


def _log(msg: str) -> None:
    print(f"[chestny-znak-mcp-ru] {msg}", file=sys.stderr, flush=True)
...
def main() -> None:
    ...
    sys.path.insert(0, str(HERE))  # чтобы импортировался crpt_mcp/ из бандла
