...
CATALOG = ROOT / "crpt_mcp" / "endpoints.yaml"
SVC = "crpt"
...
def test_entities_file_ships_with_the_catalog():
    """Карта сущностей это половина поиска и весь инструмент crpt_map. Пока
    файла не было, map молча отдавал пустоту во всех пяти серверах."""
    ents = yaml.safe_load((ROOT / "crpt_mcp" / "entities.yaml").read_text(encoding="utf-8"))
...
def test_live_questions_hit_the_right_method():
    ...
    idx = EntityIndex.load(ROOT / "crpt_mcp" / "entities.yaml")
    catalog = Catalog.from_yaml(CATALOG, entities=idx)
    for question, expected in [('коды маркировки в обороте', 'crpt_gis_cises_my'), ('проверить код', 'crpt_gis_cis_check'), ('заказ на эмиссию кодов', 'crpt_suz_order_status')]:
